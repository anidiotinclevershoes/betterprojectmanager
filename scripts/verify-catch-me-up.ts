...
async function main() {
  ...
  await check("Iron Man integration contract is explicit and shell is untouched", () => {
    ...
    assert.doesNotMatch(mode, /catch-me-up|Catch Me Up/);
    const workspace = readSrc(
      "src/components/knowledge-centre/OceanProjectWorkspace.tsx",
    );
    assert.doesNotMatch(workspace, /CatchMeUpSurface|catch-me-up/);
