...
export function obs(partial: Record<string, unknown>) {
  return {
    ...
    // PR #110 contract: missing truthIntent is rejected (not assumed current).
    truthIntent: "current",
    projectId: QUAL_ID,
...
export async function applyResolved(args: {
  ...
  /**
   * Production Review edit: CaptureSessionContext writes
   * `slice.editing ?? item.content` onto `item.content` before Apply.
   * Durable create identity is `item.content`, not Apply `text`.
   */
  reviewEdit?: string;
}) {
  ...
  for (const row of writes) {
    const item = args.reviewEdit
      ? { ...row.suggestion!, content: args.reviewEdit }
      : row.suggestion!;
