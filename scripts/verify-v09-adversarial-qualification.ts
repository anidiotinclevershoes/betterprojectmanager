...
async function main() {
  await journey(
    ...
    async () => {
      ...
      const todoTitle = todos(state).map((t) => t.title).join(" | ");
      const riskTitle = risks(state).map((r) => r.title).join(" | ");
      const label = dates(state).map((d) => d.label).join(" | ");
      const concise =
        todoTitle === "UAT script" && riskTitle === "API timeout" && label === "CAB";
      const usedStatement =
        /Please add a to-do for UAT script/i.test(todoTitle) ||
        /log API timeout/i.test(riskTitle) ||
        /put CAB on 18/i.test(label);
      if (concise) {
        return { result: "PASS", detail: "rows are UAT script / API timeout / CAB" };
      }
      if (usedStatement) {
        return {
          result: "EXPECTED_RED",
          severity: "P0",
          detail: `planner uses Apply text (statement), ignores proposedValues.title. rows: ${todoTitle} // ${riskTitle} // ${label}`,
        };
      }
      return {
        result: "UNEXPECTED_RED",
        severity: "P0",
        detail: `unexpected titles: ${todoTitle} // ${riskTitle} // ${label}`,
      };
    },
  );
