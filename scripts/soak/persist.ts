/**
 * Test-only disposable Supabase backend for the 500-capture soak.
...
export async function createSupabaseBackend(): Promise<PersistBackend> {
  ...
  const email = "hulk-live-500-soak@example.test";
  const password = "HulkLive500!pass";
  ...
  const created = await admin.auth.admin.createUser({
    ...
    password,
    email_confirm: true,
    user_metadata: { display_name: "Hulk Live 500 Soak" },
  });
