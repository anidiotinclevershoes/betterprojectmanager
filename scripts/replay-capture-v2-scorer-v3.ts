/**
 * Replay archived first-live envelopes through CURRENT production + scorer v3.
 * Not a live benchmark. Does not mutate historical v1/v2 artifacts.
 *
 *   npx tsx scripts/replay-capture-v2-scorer-v3.ts
 */
...
const POST_IDENTITY_PATH = join(ARCHIVE_DIR, "capture-v2-eval-post-identity-replay-v1.json");
const OUT_PATH = join(ARCHIVE_DIR, `${SCORER_V3_REPLAY_ID}.json`);
...
function main() {
  ...
  const beforeRescore = existsSync(HISTORICAL_RESCORE_PATH)
    ...
    : null;
  const beforePostIdentity = existsSync(POST_IDENTITY_PATH)
    ? sha256File(POST_IDENTITY_PATH)
    : null;
  ...
  const slim = {
    ...
    historicalArtifactSha256: {
      ...
      postIdentityReplayV1: beforePostIdentity,
    },
    cases: report.cases.map((row) => ({
      ...
      failureStatements: row.failureStatements,
      outcome: row.outcome,
    })),
  };
  ...
  if (beforePostIdentity && sha256File(POST_IDENTITY_PATH) !== beforePostIdentity) {
    throw new Error("Historical post-identity replay mutated during replay");
  }

  console.log(
    `Capture V2 scorer-v3 replay  id=${report.replayId} production=${report.productionSha} scorer=${report.scorerVersion} corpus=${report.corpusVersion}`,
  );
  ...
  for (const row of genuine) {
    console.log(
      `  ${row.model} ${row.caseId} run${row.runIndex} writes=${row.writeOperations.join(",") || "none"} statements=${row.failureStatements.join(" | ")}`,
    );
  }
