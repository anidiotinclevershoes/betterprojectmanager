 ...
 * Holdout rematerializes unscoped create_new + invented ids as accepted
 * creates. Adapter recovers name-only people from remaining VALIDATE
 * rejects (D-052). Capture scoped foreign_id fail-closed is unchanged.
 *
...
async function main() {
  ...
  try {
    ...
    await check(
      "VALIDATE rematerializes invented create_new ids; names still reach Organise",
      () => {
        ...
        assert.ok(
          ...
          `invented candidateTargetId codes ${staged.rejectCodes.join(",")} expected foreign_id`,
        );
        assert.ok(
          staged.validation.observations.every((obs) => obs.disposition === "create_new"),
          "rematerialized people must stay create_new",
        );
        assert.ok(
          staged.validation.observations.every((obs) => !obs.candidateTargetId),
          "invented candidateTargetId must not remain as an update target",
        );
        assert.equal(staged.provisionalCount, 2);
        assert.deepEqual(staged.draftNames, ["bob", "mike"]);
        // Bob is a complete name-only Person. Mike arrived as responsibility
        // without a structured scope, so Review may still ask — holdout
        // rematerialize preserves the name, it does not invent a scope.
        assert.equal(staged.draftNeedsReview[0], false);
        assert.ok(staged.draftNames.includes("mike"));
      },
    );

    await check(
      "POST /api/new-project keeps named people after invented candidateTargetId",
      async () => {
    ...
    await check(
      "Capture rematerializes invented create_new ids as accepted findings",
      () => {
        const run = runCaptureV2FromModelJson({
          transcript: NARRATIVE,
          rawModelJson: FOREIGN_ID,
          world: emptyWorld(),
          projectId: null,
        });
        assert.equal(run.validation.rejected.length, 0);
        assert.equal(run.validation.observations.length, 2);
        assert.ok(
    ...
    for (const row of rejectEnvelopes) {
      await check(
