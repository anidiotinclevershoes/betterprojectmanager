...
function requestedVariants(): PromptVariantId[] {
  ...
  for (const part of parts) {
    if (!ALL_VARIANTS.includes(part as PromptVariantId)) {
      throw new Error(`Unknown --variant ${part}. Use A, B, C, D, E, or ALL.`);
    }
  }
  return parts as PromptVariantId[];
}

function requestedTrials(): number {
  const idx = process.argv.indexOf("--trials");
  if (idx < 0) return 1;
  const n = Number(process.argv[idx + 1]);
  if (!Number.isInteger(n) || n < 1 || n > 8) {
    throw new Error("--trials must be an integer from 1 to 8.");
  }
  return n;
}
...
async function main() {
  ...
  console.log(
    `model=${PINNED_OPENAI_CHAT_MODEL} temperature=${PROMPT_EXPERIMENT_TEMPERATURE} variants=${variants.join(",")} trials=${trials}`,
  );
  ...
  for (const variant of variants) {
    ...
    const rows: unknown[] = [];
    const trialTotals: unknown[] = [];
    console.log(`--- variant ${variant} (${meta.label}) ---`);

    for (let trial = 1; trial <= trials; trial += 1) {
      const trialScore = emptyTotals();
    ...
    for (const item of sample) {
      ...
      trialTotals.push({ trial, totals: trialScore });
      console.log(
        `  trial ${trial} totals: recall=${trialScore.recallHits}/${trialScore.recallPossible} invent=${trialScore.inventions} foreign_id=${trialScore.foreignId} pronoun=${trialScore.unsafePronounResolution} writes=${trialScore.writes} needs_you=${trialScore.needsYou} errors=${trialScore.errors}`,
      );
    }
    ...
    console.log(
      `aggregate ${variant}: recall=${totals.recallHits}/${totals.recallPossible} invent=${totals.inventions} foreign_id=${totals.foreignId} contam=${totals.identityContamination} pronoun=${totals.unsafePronounResolution} writes=${totals.writes} needs_you=${totals.needsYou} obs=${totals.observationCount} errors=${totals.errors}\n`,
    );
  }
  ...
  const aeOnly = variants.length === 2 && variants.includes("A") && variants.includes("E");
  const liveName = aeOnly ? "prompt-experiment-holdout-ae.json" : "prompt-experiment-holdout.json";
  const committedName = aeOnly ? "holdout-results-ae.json" : "holdout-results.json";
  const livePath = join(dir, liveName);
