...
import type { JourneyMatrixRow, VerticalBoundary } from "../e2e-hosted-vertical/types";

const OUTPUT_DIR = path.join(process.cwd(), "test-results", "hosted-holdout");
...
class HostedHoldoutReporter implements Reporter {
  ...
  onEnd(): void {
    ...
    fs.writeFileSync(jsonPath, JSON.stringify({ suite: "hosted-holdout-v1", elapsedMs, rows: this.rows }, null, 2));
    const header = [
    ...
    console.log(`\nHosted holdout matrix written to ${mdPath}\n`);
    // eslint-disable-next-line no-console
    console.log([...header, ...lines].join("\n"));
  }
}

export default HostedHoldoutReporter;
