...
**Last housekeeping:** 11 September 2026 (D-052 New Project Organise silently drops schema-rejected people; D-050 hosted schema catch-up merged; D-049 hydrate completeness: authoritative Knowledge section lists no longer `.slice(0, 24)`)  
...
| **Evidence / repro** | Hosted `POST /api/new-project` 200 with empty `draft` + `provisionalItems: []`. Deterministic: `scripts/verify-np-organise-observation-loss.ts`. Raw model JSON was not logged (provenance never stores content). |
...
| **Related docs** | People rules (name-only Person is complete); PR #155 Preview Organise empty-draft reproduction |
