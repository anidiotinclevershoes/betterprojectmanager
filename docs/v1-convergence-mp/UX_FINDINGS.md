...
Findings are from **frozen V2 product + live Magic Patterns v8 + 19 Aug visual constitution + stated architecture**.
...
Recommended deterministic conventions (unchanged):

1. Domain frames are visually **primary**. Knowledge prose that is only a projection is quieter or omitted when a domain row exists.
2. Historical evidence lives in More details and History — not as a peer card in the current-truth column.
3. Needs you must not use the same chrome as Known (`TrustMark` already does this in MP).
...
Flag for architecture: unify on canonical-truth / domain stores. UX follows; do not paint a second “source of truth” chip ontology.

**“Accept as known”** in the MP Why block is prototype-local. Shipping it as a KC control would bypass Capture Review / Phase 3B. Flag, do not implement as a write.
...
Do **not** copy MP “Accept as known” into Capture Review. Review already has Approve / Apply Ready.
...
Do not invent a marketing onboarding carousel and call it MP. Do not treat NP categorisation as onboarding.

Paste: constitution says out of V1; choose UI already dropped it; API still accepts paste. PO: delete remnants or keep as hidden power path.

## 5. Onboarding (unresolved — do not choose)

Unchanged. Live MP does not draw first-run.

**Smallest PO decision:** keep first-run = New Project Talk, **or** add one disposable sample that is visually marked as example and excluded from Capture identity matching.
...
| Keyboard | MP Escape = back then close; DatePopover Escape | Product drawer: Escape, initial focus on Close. Confirm Owner is a dialog. Coach auto-open (D-031) still steals Capture. |
...
**MP primitives to extract later (after architecture + PO frame-order):**
...
- `/coaching` + Coach auto-open; unmounted `HeaderCoachButton`
- Duplicate **Timeline** embed (still in **both** MP v8 and product)
...
1. **PO:** confirm v8 frame order as V1 default (or keep constitution/product Current-position-first); decide onboarding; decide Desert-vs-constitution text; decide that “Accept as known” is **not** a KC write.
2. **Architecture reconciliation** (sibling): current-truth authority, Person workspace scope, leftover prose (D-030), Waiting authority, milestone complete. UX follows.
3. **D-032:** one Capture / New Project engine so Review and NP have one language.
4. **Token hygiene:** semantic status colours shared Ocean/Desert; no screen forks.
5. **Evolve existing drawer** toward the compact overlay contract (do not reflow KC; full-width mode bar).
6. **KC current-truth chrome:** stop leftover prose competing (depends on architecture). Apply confirmed frame order.
7. **Capture Review disclosure:** show merged/commentary as optional; keep default light. (No MP screen — follow product + philosophy.)
8. **New Project categorisation visual** — behaviour already exists; still no MP drawing.
9. **Coach overlay (D-031)** and leftover `/coaching`.
10. **Then** remaining deltas, screen-by-screen, using existing cards/drawer. Kill duplicate Timeline when dates settle.
...
1. Knowledge Centre — Ocean — resting (v8 order **or** constitution order, once PO confirms)
...
Optional: Account Appearance (Ocean / Desert); narrow viewport KC + mode bar with inspector open; zero-project first-run if onboarding is chosen.
...
- Is Person becoming workspace-scoped in V1 or later?
- Milestone complete: Needs you forever, or a real status?
- Is “Accept as known” from the inspector allowed as a 3B write, a Capture Review jump, or never?
...
**Debt this design would create if implemented blindly from MP v8:**
...
Those are reasons **not** to implement the overhaul from this PR.
