...
**Date:** 28 August 2026  
**Merged main:** `2e024d0bd04db87e7375a4c5b0106ccf4d4de31a` (integration PR #95)  
...
**Same-browser logout → login without a full remount can still display the previous user’s in-memory workspace.** Authenticated APIs still fail-closed. Testers on their own devices do not hit this. It is a **V1 MUST** repair before anyone shares a browser, and it blocks calling live two-account isolation fully proven. See D-036.

Do not reopen Capture. Do not start V1 architecture. Put Lume in testers’ hands on separate devices, or land the session-switch remount first.
...
| Live two-account **session-switch UI** | **Fail** — D-036 (in-memory MissionState survives SPA login) |

v0.9 may be used by **trusted testers on their own browsers**.

It is **not** fully closed for the two-account isolation gate until D-036 is repaired (smallest remount/rehydrate slice). See §11.
...
This is not an RLS/IDOR break. Mutations still fail closed. It **is** cross-account truth **display** on a shared browser.

### Isolation verdict

**FAIL — LIVE TENANT ISOLATION BLOCKER** (session-switch display)

Server/API/RLS isolation: **proven**.

Smallest bounded repair (do **not** redesign the data layer):

1. On logout: reset `MissionState` to empty and clear persist meta; **or**
2. On `SIGNED_IN`, re-hydrate whenever `session.user.id` ≠ last hydrated user (ignore `hydrateSucceeded`); **or**
3. Login/logout via `window.location` full load so `MissionProvider` remounts.

Any one of those is enough. Prefer (2)+(3).
...
Ship to trusted testers (separate browsers until D-036). Watch the questions in §7.
...
| Item | Why this stage |
| --- | --- |
| D-036 session-switch remount | Shared-device display of another user’s workspace |
...
| D-036 session-switch display | In-memory MissionState survives SPA login | **V1 MUST** | Live A9: B UI painted for A; APIs 404 | Remount/rehydrate — see §6 |
...
| D-039 `openaiConfigured` stale | 401 on `/login` can stick | V1 SHOULD | Smoke; rides with D-036 | Re-probe after SIGNED_IN |
...
> **v0.9 Capture is frozen. Server-side tenant isolation is proven. Trusted testers on their own browsers may begin learning.**
>
> **Do not declare LUME v0.9 CLOSED until D-036 (session-switch remount) is repaired.** Shared-browser SPA login can still display another user's in-memory workspace. Authenticated APIs still fail-closed.
>
...
>
> **D-036 is the remaining live isolation repair. It is not a Capture unfreeze.**
