...
export function composeKnowledgeCentreItems(
    ...
    remember(
      withSearch({
        ...
        needsYou: person.unconfirmed
          ? `Needs You — Confirm what ${person.name} is responsible for.`
          : null,
        icon: KC_BUCKET_ICON.people,
        tagKind: "stakeholder",
  ...
  // Structured dates that are not yet domain timeline rows. Presentation only —
  // does not change ocean-frames / search-authority builders.
  for (const item of knowledge.structured ?? []) {
    if (item.lifecycle !== "current" || item.kind !== "date") continue;
    const title = item.meta?.date?.label?.trim() || item.body;
    if (!title || dateSeen.has(title.toLowerCase())) continue;
    dateSeen.add(title.toLowerCase());
    remember(
      withSearch({
        id: `date:${item.id}`,
        bucket: "knowledge",
